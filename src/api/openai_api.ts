...
/**
 * Sends an image to the OpenAI API to detect if it's an artifact or valuable content.
...
export async function detectArtifactsInImage(
  ...
  try {
    ...
    if (!content) {
      console.warn('⚠️ No content received from OpenAI API for artifact detection');
      return null;
    }
    ...
    if (!parsedResponse) {
      console.warn('⚠️ Failed to parse artifact detection response from OpenAI API');
    ...
    const error = e instanceof Error ? e : new Error(String(e));
    console.error(`⚠️ Error communicating with OpenAI API for artifact detection: ${error.message}`);
    return null;
  }
}
